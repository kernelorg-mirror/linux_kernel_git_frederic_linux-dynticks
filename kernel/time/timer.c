					     ...
					     bool *idle)
{
	...
	/*
	 * Set base->is_idle only when caller is timer_set_idle()
	 */
	if (idle) {
		/*
...
{
	u64 expires = __get_next_timer_interrupt(basej, basem, NULL);
...
{
	u64 expires = __get_next_timer_interrupt(basej, basem, idle);
